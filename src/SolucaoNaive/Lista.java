...
public class Lista
{
	...
	protected class Elo
	{
		...
		public Elo(Cluster elem)
		{
			dado = elem;
			prox = null;
		}
		
		public Elo(Cluster elem, Elo proxElem)
		{
			dado = elem;
			prox = proxElem;
		}
	...
	public void insere(Cluster novo)
	{
		Elo p = new Elo(novo);
		p.prox = prim;
		prim = p;
	...
	public boolean busca(Cluster elem)
	{
		Elo p;
		
		for(p = prim; p != null; p = p.prox)
		{
			if(p.dado.compareTo(elem) == 0)
				return true;
		}
	...
	public boolean buscaRecursiva(Cluster elem)
	{
		if(this.vazia())
			return false;
	...
	private boolean buscaRecursiva(Cluster elem, Elo p)
	{
		if(p == null)
			return false;
	...
	public boolean remove(Cluster elem)
	{
		Elo p;
	...
	*/
	...
	public Lista separa(Cluster n)
	{
		...
		for(p = prim; (p != null) && (p.dado.compareTo(n) != 0); p = p.prox);
	...
	public static Lista constroi(Cluster[] v, int n)
	{
		Lista l = new Lista();
		Elo p, ult;
